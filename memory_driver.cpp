...
int main(int argc, char *argv[]) // the program runs like this: ./program <filename> <mode>
{
	...
	double L1_miss_rate, VC_miss_rate, L2_miss_rate, AAT;
	// compute the stats here:
	Stat myStat = myCache.get_stats();
	// cout << "myStat " << myStat.missL1 << " " << myStat.accL1 << " " << myStat.missL2 << " " << myStat.accL2 << " " << myStat.missVic << " " << myStat.accVic << endl;
	L1_miss_rate = (double)myStat.missL1 / myStat.accL1;
	VC_miss_rate = (double)myStat.missVic / myStat.accVic;
	L2_miss_rate = (double)myStat.missL2 / myStat.accL2;

	AAT = 1 + L1_miss_rate * (1 + (VC_miss_rate * (8 + (L2_miss_rate * 100))));
	cout << std::setprecision(10)
		 << "(" << L1_miss_rate << "," << L2_miss_rate << "," << AAT << ")" << std::endl;
